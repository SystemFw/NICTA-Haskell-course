...
instance Functor f => Functor (StateT s f) where
  (<$>) ::
    ...
    -> StateT s f b
  (<$>) = inStateT . (<$>) . (<$>) . first
    where inStateT f = StateT . f . runStateT
...
-- [(3,3)]
getT ::
  Monad f => -- Applicative seems to be enough
  StateT s f s
getT =
  StateT $ pure . join (,)
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF x = evalT (filtering f x) S.empty
  where f e = StateT $ \s -> if e > 100
                             then Empty
                             else Full (S.notMember e s, S.insert e s)  
...
instance Functor f => Functor (OptionalT f) where
  (<$>) = inOpt .(<$>).(<$>)
    where inOpt f = OptionalT . f . runOptionalT
...
instance Monad f => Monad (OptionalT f) where
  k =<< (OptionalT x) = OptionalT $ (\op -> case op of           
                                       Empty -> pure Empty                   
                                       (Full a) -> runOptionalT $ k a) =<< x 
