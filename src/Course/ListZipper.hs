...
instance Functor ListZipper where
  f <$> ListZipper ll v rr = ListZipper (f <$> ll) (f v) (f <$> rr)
...
withFocus ::
  ...
  -> ListZipper a
  -> ListZipper a
withFocus f (ListZipper ll v rr) = ListZipper ll (f v) rr
...
hasLeft ::
  ListZipper a
  -> Bool
hasLeft (ListZipper ll _ _) = not $ isEmpty ll
...
hasRight ::
  ListZipper a
  -> Bool
hasRight (ListZipper _ _ rr) = not $ isEmpty rr
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper ll v rr) =
  case break p ll of
  (_,Nil) -> IsNotZ
  (xs,y:.ys) -> IsZ $ ListZipper ys y (xs ++ v:.rr)  
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper ll v rr) =
  case break p rr of
  (_,Nil) -> IsNotZ
  (xs,y:.ys) -> IsZ $ ListZipper (reverse xs ++ v:.ll) y ys
...
moveLeftLoop ::
  ListZipper a
  -> ListZipper a
moveLeftLoop (ListZipper Nil v rr) = ListZipper ll nv  Nil
  where nv:.ll = reverse $ v:.rr
moveLeftLoop (ListZipper (l:.ls) v rr) = ListZipper ls l (v:.rr) 
...
moveRightLoop ::
  ListZipper a
  -> ListZipper a
moveRightLoop (ListZipper ll v Nil) = ListZipper Nil nv rr
  where nv:.rr = reverse $ v:.ll
moveRightLoop (ListZipper ll v (r:.rs)) = ListZipper (v:.ll) r rs
...
moveLeft ::
  ListZipper a
  -> MaybeListZipper a
moveLeft lz@(ListZipper l _ _)
  | isEmpty l = IsNotZ
  | otherwise = IsZ $ moveLeftLoop lz
...
moveRight ::
  ListZipper a
  -> MaybeListZipper a
moveRight lz@(ListZipper _ _ r)
  | isEmpty r = IsNotZ
  | otherwise = IsZ $ moveRightLoop lz
...
swapLeft (ListZipper (l:.ls) v rr) = IsZ $ ListZipper (v:.ls) l rr
...
swapRight (ListZipper ll v (r:.rs)) = IsZ $ ListZipper ll r (v:.rs)
...
dropLefts ::
  ListZipper a
  -> ListZipper a
dropLefts (ListZipper _ v rr) = ListZipper Nil v rr
...
dropRights ::
  ListZipper a
  -> ListZipper a
dropRights (ListZipper ll v _) = ListZipper ll v Nil
...
moveLeftN n
  | n == 0 = IsZ
  | n < 0 = moveRightN $ abs n 
  | otherwise = asMaybeZipper (moveLeftN $ pred n) . moveLeft
-- See moveRightN for another way of doing this
...
moveRightN n
  | n == 0 = IsZ
  | n < 0 = moveLeftN $ abs n
  | otherwise = (P.!! n) . P.iterate (moveRight -<<) . IsZ 
...
nth ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
nth n = moveLeftN <$> P.subtract n . length . lefts <*> id
...
-- prop> rights (end lz) == Nil
  ...
  ListZipper a
  -> ListZipper a
end = (P.!!) <$> P.iterate moveRightLoop <*> length . rights
-- For another way to do this, look at start
...
deletePullLeft (ListZipper (l:.ls) _ rr) = IsZ $ ListZipper ls l rr
...
deletePullRight (ListZipper ll _ (r:.rs)) = IsZ $ ListZipper ll r rs
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft nv (ListZipper ll v rr) = ListZipper (v:.ll) nv rr 
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight nv (ListZipper ll v rr) = ListZipper ll nv (v:.rr)
...
instance Extend ListZipper where
   f <<= lz = ListZipper (gen moveLeft lz) (f lz) (gen moveRight lz)
     where gen dir = unfoldr $ (<$>) (f &&& id) . toOptional . dir
...
instance Traversable ListZipper where
  traverse f (ListZipper ll v rr) =
    (ListZipper . reverse) <$> traverse f (reverse ll) <*> f v <*> traverse f rr
-- The first list in a zipper is normally reversed for efficiency, but we need to traverse it in the original order when dealing with effects
